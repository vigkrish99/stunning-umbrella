...
export async function GET(request: NextRequest) {
  try {
    ...
    // Aggregate underperformers (lowest rotation rate) for the period
    ...
    const underperformers = await RotationMetric.aggregate([
      { $match: matchStage },
      { $sort: { rotationRate: 1 } },
      { $limit: limit },
      {
    ...
    const results = underperformers.map((r) => {
    ...
    console.error("Underperformers API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch underperformers" },
      { status: 500 }
    );
  }
}
